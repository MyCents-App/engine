...
working. Our own page POSTs the raw image bytes as the request body; /extract also accepts a
multipart/form-data upload (see parse_multipart_image), because a separately-written frontend
will normally reach for FormData + fetch.
...
# --- CORS -------------------------------------------------------------------------------
# The frontend is served from a different origin (a laptop, or a deployed page) and reaches
# this server through the public tunnel, so every response needs an Access-Control-Allow-Origin
# header and the preflight OPTIONS must be answered -- without both, the browser refuses to
# even send the real POST. "*" is right for a public demo endpoint that holds no user data;
# set DEMO_ALLOW_ORIGIN to a specific origin to narrow it.
ALLOW_ORIGIN = os.environ.get("DEMO_ALLOW_ORIGIN", "*")
# Content-Type is all a plain fetch needs. X-Demo-Key is pre-allowed so a shared-secret check
# can be switched on later without the frontend having to change anything else.
...
def parse_multipart_image(body: bytes, content_type: str) -> tuple[bytes, str] | None:
    """Pull the first file part out of a multipart/form-data body.

    Why this exists: /extract originally required the raw image bytes as the request body,
    which is what our own phone page sends. But the normal way to upload from a frontend is
    FormData + fetch, which wraps the bytes in a multipart envelope -- and Surya would then
    be handed the envelope instead of an image. Accepting both means the frontend can use
    whichever it already has, with no coordination needed.

    Uses email.parser rather than cgi.FieldStorage: cgi is deprecated in 3.11 and removed in
    3.13. Re-attaching the Content-Type header is what gives the parser the boundary.

    Returns (image_bytes, content_type) for the first part that looks like a file, or None.
    """
    header = ("Content-Type: " + content_type).encode("utf-8")
    blank = bytes((13, 10, 13, 10))  # CRLF CRLF, written without escapes on purpose
    msg = BytesParser(policy=email_policy).parsebytes(header + blank + body)
    ...
    for part in msg.iter_parts():
        ...
        if not payload:
            continue
        # A file part has a filename; some clients omit it but still set an image type.
        if part.get_filename() or part.get_content_type().startswith("image/"):
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _cors_headers(self) -> None:
        ...
        if ALLOW_ORIGIN != "*":
            # Tells caches the response varies by requesting origin.
    ...
    def do_OPTIONS(self):
        """CORS preflight. A cross-origin POST with a JSON or image content type is preceded
        by this; if it does not get a 2xx the browser never sends the actual request."""
    ...
    def do_POST(self):
        ...
        raw_ctype = self.headers.get("Content-Type") or "image/jpeg"
        ctype = raw_ctype.split(";")[0].strip().lower()

        if ctype == "multipart/form-data":
            parsed = parse_multipart_image(body, raw_ctype)
            if parsed is None:
                self._json(400, {"error": "multipart body contained no file part."})
                return
            image, ctype = parsed
        else:
            image = body
        ...
        try:
            ...
            # Must be caught BEFORE URLError, which it subclasses. Otherwise a perfectly
            # healthy OCR service returning 500 on an undecodable upload gets reported as
            # "is ocr_service.py running?", and you go restart a service that was never
            # the problem -- an expensive wrong turn in the middle of a demo.
            detail = exc.read().decode("utf-8", "replace").strip()[:300]
            self._json(502, {"error": f"The OCR service rejected that image "
                                      f"(HTTP {exc.code}). Is it actually a photo? {detail}"})
            return
...
def warmup() -> None:
    """Run one throwaway generation before the server starts accepting requests.

    The first generate() call on a freshly loaded model pays for CUDA kernel selection and
    lazy allocator setup -- measured on this box at roughly 5 seconds on top of the usual
    ~3s. Spending it at startup means the first receipt of the demo is as fast as the tenth,
    which matters when the first one is the one someone is watching.
    """
    t0 = time.time()
    try:
        with _lock:
            extract("MINI MART\nwater  10.00\nTotal  10.00")
        print(f"Warm-up generation done in {time.time() - t0:.1f}s", flush=True)
    except Exception as exc:  # noqa: BLE001
        print(f"WARNING: warm-up failed ({type(exc).__name__}: {exc}); "
              "the first real request will just be slower.", flush=True)
