...
Containerized deployment on a GPU host. For running the pair of services
straight from a checkout on a Windows workstation — which is what the demos
have used — see [`../RUN.md`](../RUN.md) instead; this file is the Docker path.
...
## 2. What the build needs from outside the image

**a) The SuryaOCR project** (the `receipt_ocr` package). `ocr_service.py`
imports it. On this branch it is already in the repo as the sibling folder
`../SuryaOCR`, so `SURYA_PROJECT_PATH=../../SuryaOCR` in `docker/.env` (paths
there resolve relative to `docker-compose.yml`) needs no further setup.

> On the standalone `extractEngine` branch it is not present, and has to be
> materialized from the `suryaOCR` branch first:
...
Two projects rather than one venv, on purpose: the OCR stage pins
`transformers<5` and has its own `pyproject.toml`, and merging the two
dependency sets into one interpreter is exactly the conflict the two-container
split exists to avoid.

**b) The fine-tuned checkpoint.** Not in git — it is multi-GB and changes on a
different cadence than the code. Mounted read-only at `/models`. Set
...
# a long receipt, IN CAPTURE ORDER — overlapping frames are fine and wanted
curl -X POST https://your-domain.ngrok.app/v1/extract \
     -H "X-API-Key: $ENGINE_API_KEY" \
...
# replay a multi-photo receipt — the only way to exercise the stitching
# without a camera
curl -X POST https://your-domain.ngrok.app/v1/extract-text \
     -H "X-API-Key: $ENGINE_API_KEY" -H "Content-Type: application/json" \
     -d '{"pages": ["7-ELEVEN\nมาม่า 18.00", "มาม่า 18.00\nรวม 18.00"]}'
```
...
### Page order matters; overlap does not

Pages are OCR'd separately, then **stitched**: the leading lines of each page
that repeat the trailing lines of the ones before it are dropped, and the model
is called once on the joined text (`app/stitch.py`). Users should overlap their
shots — it is the only way to guarantee no line falls in the gap between
frames — and the repetition costs nothing.

Order, though, is not recoverable: the search only looks for the start of page
N at the end of pages 1..N-1. Never let the client reorder the images.

`ENGINE_STITCH_PAGES=0` turns the stitching off and concatenates instead. That
duplicates every item in the overlap and is only there to compare against.
Measured behaviour and the threshold tuning: `reports/stitch_overlap.md`.
...
reports the actual token count and how many photos produced it.

`ENGINE_MAX_SEQ_LENGTH` defaults to 4096 rather than the 2048 the QLoRA
fine-tune was trained at. Training length caps what the model learned from,
not what it can be served with, and Qwen3.5's own context is far longer than
either — the eval numbers are unaffected, since every eval prompt was under
1000 tokens. At 2048 the budget is ~1280, which a single dense receipt already
reaches, so a multi-photo receipt would 413 on exactly the case the multi-photo
path exists for. Raise it further if the GPU has headroom, and re-measure VRAM.
