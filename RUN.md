...
| `ENGINE_MAX_SEQ_LENGTH` | 2048 | Prompt budget is this minus `MAX_NEW_TOKENS` |
...
No GPU needed. Date parsing and post-processing only:

```bash
uv venv .venv-test --python 3.11
uv pip install --python .venv-test/Scripts/python.exe pytest
cd train && ../.venv-test/Scripts/python.exe -m pytest tests/ -q
```
